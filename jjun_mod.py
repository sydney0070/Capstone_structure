...
def build_model():
    ...
    soil_node_start = 10000

    soil_surface_node = soil_node_start
    soil_nodes.append(soil_surface_node)
    ops.node(soil_surface_node, 0.0, 0.0)   # 지표면 지반 노드

    cumulative_depth = 0.0
    for i, layer in enumerate(SOIL_LAYERS):
        cumulative_depth += layer["thickness"]

        snid = soil_node_start + 1 + i
    ...
    for col in range(NUM_BAYS + 1):
        fn = node_id[(0, col)]
        ops.element('zeroLength', ele_id, fn, soil_surface_node, '-mat', 99, '-dir', 1)
        ele_id += 1
...
def time_history_analysis(node_id, soil_nodes, t_arr, acc_arr):
    ...
    for step in range(n_steps):
        ...
        # 지반 변위
        results["soil_disp"].append(ops.nodeDisp(soil_nodes[0], 1))
...
# 구조물 형상 HTML 시각화
# ─────────────────────────────────────────────────────────────────────
def visualize_model_html(out_path=None):
    """
    현재 OpenSees Domain에 생성된 노드와 요소를 HTML로 시각화.
    - 구조물 요소: 파란색 계열
    - 지반 요소: 갈색 계열
    - SSI / zeroLength 연결 요소: 빨간색 계열
    """

    if out_path is None:
        out_dir = "/Users/sydney/Desktop/capstone/outputs"
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(out_dir, "model_shape.html")
    ...
    for ele in ele_tags:
        ...
        # 요소 분류
        if ni >= 10000 and nj >= 10000:
            color = "saddlebrown"
            width = 5
            name = "Soil element"
        elif (ni >= 10000 and nj < 10000) or (ni < 10000 and nj >= 10000):
            color = "red"
            width = 3
            name = "SSI / zeroLength"
        else:
        ...
        fig.add_trace(go.Scatter(
            ...
            hoverinfo="text",
            text=f"Element {ele}<br>Node {ni} → Node {nj}<br>{name}",
            showlegend=False
        ))
    ...
    for nd, (x, y) in node_coords.items():
        x_nodes.append(x)
        y_nodes.append(y)
        text_nodes.append(f"Node {nd}<br>x={x:.3f}, y={y:.3f}")
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[str(nd) for nd in node_coords.keys()],
    ...
    fig.update_layout(
        title="OpenSeesPy Model Shape: RC Frame + Soil Column",
        xaxis_title="X [m]",
        yaxis_title="Y [m]",
        width=1000,
        height=750,
    ...
    print(f"  HTML 모델 형상 저장 완료: {out_path}")
...
def visualize_results(t_arr, acc_arr, results, assessment, max_drifts,
    ...
    out_path = os.path.join(out_dir, "seismic_analysis_result.png")
...
if __name__ == "__main__":
    ...
    print("━━━ 모델 형상 HTML 시각화 ━━━")
    model_html_path = visualize_model_html()
